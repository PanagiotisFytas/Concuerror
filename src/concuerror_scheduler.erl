...
plan(State) ->
  ...
  case HasMore of
    true ->
      ...
      true = own_next_interleaving(NewState),
      Controller ! {has_more, self(), make_state_transferable(NewState), Duration, IE+1};
...
size_of_backtrack_transferable([TraceState|Rest]) ->
  size_of_backtrack_transferable_aux(TraceState#trace_state_transferable.wakeup_tree) + size_of_backtrack_transferable(Rest).
...
get_next_event(#scheduler_state{
  ...
  case WakeupTree of
    ...
    _ -> 
      case split_wut_at_owned(WakeupTree) of
...
insert_wakeup_optimal_parallel(SleepSet, Wakeup, V, Bound, Origin, Ownership) ->
  case has_initial(SleepSet, V) of
    true -> skip;
    false -> insert_wakeup_parallel(Wakeup, V, Bound, Origin, Ownership)
  end.
  ...
  case check_initial(Event, NotDep) of
    ...
    NewNotDep ->
      case Deeper =:= [] of
        true  -> skip;
        false ->
          case insert_wakeup_parallel(Deeper, NewNotDep, Bound, Origin, Ownership) of
            Special
              when
                Special =:= skip;
                Special =:= over_bound -> Special;
...
find_prefix_parallel_optimal([TraceState|Rest]) ->
  ...
  case Tree =:= [] of
    ...
    false ->
      case split_wut_at_owned(Tree) of
        {Tree, []} ->
          %% no owned entries
          find_prefix_parallel_optimal(Rest);
...
initialize_execution_tree_aux([TraceState, NextTraceState|Rest]) ->
  ...
  #execution_tree{
     children = 
       [initialize_execution_tree_aux([NextTraceState|Rest])] ++ wut_to_exec_tree(NextWuT),
...
print_tree(Prefix, ExecTree) ->
  ...
  io:fwrite("~sNode: ~p~n", [Prefix, Event]),
...
print_all_wut_rev([]) ->
  io:fwrite("~n", []);
print_all_wut_rev([#trace_state{} = TraceState|Rest]) ->
  [Ev|_] = TraceState#trace_state.done,
  io:fwrite("~w:TraceState: ~p ~p~n", [TraceState#trace_state.index, Ev#event.actor, Ev#event.event_info]),
  io:fwrite("WuT:~n"),
...
print_all_wut_rev([TraceState|Rest]) ->
  [Ev|_] = TraceState#trace_state_transferable.done,
  io:fwrite("TraceState: ~p~n", [Ev#event_transferable.actor]),
  io:fwrite("WuT:~n"),
  print_wut(TraceState#trace_state_transferable.wakeup_tree),
  %% io:fwrite("SleepSet: ~w~n", [TraceState#trace_state.sleep_set]),
  print_all_wut_rev(Rest).
...
print_wut(_, []) ->
  ok;
print_wut(Prefix, [#backtrack_entry{} = Entry|Rest]) ->
...
print_wut(Prefix, [Entry|Rest]) ->
  ...
  Ownership = Entry#backtrack_entry_transferable.ownership,
  io:fwrite("~s~p ~p~n", [Prefix ++ "|-> ", Actor, Ownership]),
  Pad =
    case Rest of
      [] ->
  ...
  print_wut(Prefix ++ Pad, Entry#backtrack_entry_transferable.wakeup_tree),
  print_wut(Prefix, Rest).
  ...
  case logically_equal(NextActiveEvent, OldNextActiveEvent) of
    true ->
      ...
      {Prefix, [NextChild|Suffix]} = 
        try 
          {P, [NC|S]} = split_children(NextActiveEvent, WuTInsertedChildren),
          {P, [NC|S]}
        catch _:_ ->
    ...
    false ->
      {[UpdatedNextTraceState|UpdatedRest], TraceInsertedChildren, EntriesRemoved} =
        case split_children(NextActiveEvent, WuTInsertedChildren) of
          {Prefix, [Child|Suffix]} ->
            {[UpNextTS|UpRest],
             UpdatedChild,
             ER} = 
              insert_new_trace_aux([UpdatedNextTraceState|Rest], Child), 
...
-spec insert_new_trace(reduced_scheduler_state(), execution_tree()) ->
                          execution_tree().

insert_new_trace(Fragment, ExecutionTree) ->
  #reduced_scheduler_state{
     trace = Trace
    } = Fragment,
  NewExecutionTree =
...
insert_new_trace_aux([TraceState], ExecutionTree) ->
  %%exit(impossible6),
  #trace_state_transferable{
     done = Done
    } = TraceState,
  ...
  ExecutionTree;
insert_new_trace_aux([TraceState, NextTraceState|Rest], ExecutionTree) ->
  #trace_state_transferable{
     done = Done
    } = TraceState,
  #trace_state_transferable{
...
split_wut_transferable([Entry|Rest], OwnedWuT, NotOwnedWuT, DisputedWuT) ->
  Ownership = determine_ownership(Entry),
  case Ownership of
    owned->
...
get_ownership(DisputedWuT, [], NewOwnedWuT, NotOwnedWuT) ->
  OwnedWut = fix_ownership(DisputedWuT, disputed, owned),
  {OwnedWut ++ NewOwnedWuT, NotOwnedWuT};
get_ownership([], _, NewOwnedWuT, NotOwnedWuT) ->
  {NewOwnedWuT, NotOwnedWuT};
get_ownership([DisputedEntry|Rest], Children, NewOwnedWuT, NotOwnedWuT) ->
  case get_ownership_aux(DisputedEntry, Children) of
    {owned, OwnedEntry, ChildrenLeft} ->
      get_ownership(Rest, ChildrenLeft, [OwnedEntry|NewOwnedWuT], NotOwnedWuT);
    {not_owned, NotOwnedEntry, ChildrenLeft} ->
      get_ownership(Rest, ChildrenLeft, NewOwnedWuT, [NotOwnedEntry|NotOwnedWuT])
  end.
...
get_ownership_aux(
  ...
  case split_children(Event, Children) of
    {Prefix, [Child|Suffix]} ->
      [NotOwnedEntry] = fix_ownership([DisputedEntry], disputed, not_owned),
      {not_owned, NotOwnedEntry, Prefix ++ Suffix};
    {Children, []} ->
      [OwnedEntry] = fix_ownership([DisputedEntry], disputed, owned),
      {owned, OwnedEntry, Children}
  end;  
get_ownership_aux(
  #backtrack_entry_transferable{
     event = Event,
  ...
  case split_children(Event, Children) of
    {Prefix, [Child|Suffix]} ->
      {NewOwnedWuT, NewNotOwnedWuT} =
        get_ownership(WuT, Child#execution_tree.children, [], []),
      FixedEntry =
        DisputedEntry#backtrack_entry_transferable{
          wakeup_tree = NewOwnedWuT ++ NewNotOwnedWuT
         },
      case NewOwnedWuT of
        [] ->
          {not_owned, FixedEntry, Prefix ++ Suffix};
        _ ->
          {owned, FixedEntry, Prefix ++ Suffix}
      end;
    {Children, []} ->
      [OwnedEntry] = fix_ownership([DisputedEntry], disputed, owned),
      {owned, OwnedEntry, Children}
...
fix_ownership([], _, _) ->
  [];
fix_ownership([Entry|Rest], From, To) ->
  #backtrack_entry_transferable{
     ownership = Ownership,
     wakeup_tree = WuT
    } = Entry,
  ...
  FixedEntry =
    Entry#backtrack_entry_transferable{
      ownership = FixedOwnership,
      wakeup_tree = fix_ownership(WuT, From, To)
     },
  [FixedEntry|fix_ownership(Rest, From, To)].
...
insert_wut_into_children([Entry|Rest], Children) ->
  ...
  Prefix ++ [UpdatedOrNewChild|insert_wut_into_children(Rest, Suffix)].
...
-spec update_trace_from_exec_tree(reduced_scheduler_state(), execution_tree()) ->
                                     reduced_scheduler_state().

update_trace_from_exec_tree(Fragment, ExecutionTree) ->
  #reduced_scheduler_state{
     trace = Trace
    } = Fragment,
  RevNewTrace =
...
update_trace_from_exec_tree_aux([TraceState], ExecutionTree) ->
  #trace_state_transferable{
     done = Done,
     sleep_set = _Sleep
    } = TraceState,
  ...
  true = logically_equal(ActiveEvent2, ActiveEvent),
  [TraceState];
update_trace_from_exec_tree_aux([TraceState, NextTraceState|Rest], ExecutionTree) ->
  #trace_state_transferable{
     done = Done,
     sleep_set = _Sleep
    } = TraceState,
  ...
  true = logically_equal(ActiveEvent2, ActiveEvent),
  [NextActiveEvent|_NextFinishedEvents] = NextDone,
  AllSleep = NextDone,%%ordsets:from_list([NextActiveEvent|NextSleep]),
  ...
  [UpdatedNextTraceState|UpdatedRest] =
    case split_children(NextActiveEvent, Children) of
      {Prefix, [Child|Suffix]} ->
        update_trace_from_exec_tree_aux([UpdatedNextTraceState|Rest], Child);
      {Children, []} ->
        %% Rest must contain no wakeup tree
        [UpdatedNextTraceState|Rest]
    end,
  [TraceState, UpdatedNextTraceState|UpdatedRest].
    ...
    {Prefix, [EqualEntry|Suffix]} ->
      NewEntry =
        EqualEntry#backtrack_entry_transferable{
          ...
          ownership = Ownership,
          wakeup_tree =
            reclaim_ownership(WuT, EqualEntry#backtrack_entry_transferable.wakeup_tree)
         },
...
execution_tree_to_wut([Node|Rest]) ->
  ...
  Entry =
    #backtrack_entry_transferable{
       event = Event,
       wakeup_tree = execution_tree_to_wut(Children)
      },
...
update_execution_tree_done_aux([TraceState, NextTraceState|Rest], ExecutionTree) ->
  ...
  %% {Prefix, [NextChild|Suffix]} = split_children(NextActiveEvent, ActiveChildren),
  
  %% {Prefix, [NextChild|Suffix]} =
  %%   split_children(NextActiveEvent, ActiveChildren),
  %% {UpdatedActiveChildren, MaybeNewFinishedChild} =
  ...
  DisputedWuT = [],
  {UpdatedActiveChildren, MaybeNewFinishedChild} =
    case split_children(NextActiveEvent, ActiveChildren) of
      {Prefix, [NextChild|Suffix]} ->
        case update_execution_tree_done_aux([NextTraceState|Rest], NextChild) of
          {node_finished, Event} ->
            exit(impossible),
...
%%      io:fwrite("5b~n"),
      ...
      GivenEntries =
        case ?granularity of
          coarse ->
            [OwnedEntry];
          fine ->
            distribute_wut(OwnedEntry)
        end,
      %% io:fwrite("7: #GivenEntries~w~n", [length(GivenEntries)]),
      ...
      %% io:fwrite("8: #GivenEntries~w~n", [length(NewFragmentTraces)]),
      %% NewFragmentTrace = [NewFragmentTraceState|RevTracePrefix],
...
distribute_wut(Entry) ->
  ...
  [Entry#backtrack_entry_transferable{wakeup_tree = merge_with_pre(WuT,DistributedEntry)} ||
    DistributedEntry <- DistributedWuT].
  %% [Entry#backtrack_entry_transferable{wakeup_tree = En} ||
  %%   En <- keep_prev_entries(DistributedWuT, [])].

merge_with_pre(WuT, DistributedEntry) ->
  case split_wut_with(DistributedEntry#backtrack_entry_transferable.event, WuT) of
    {Prefix, [En|Suffix]} ->
      Prefix ++ [DistributedEntry];
    {WuT} ->
      exit(impossible8)
  end.
